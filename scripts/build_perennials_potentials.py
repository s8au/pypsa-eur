...
def build_nuts2_shapes():
    ...
    """
    nuts2_file = 'data/nuts/NUTS_RG_01M_2021_4326_LEVL_2.geojson'
    nuts2 = gpd.GeoDataFrame(
        gpd.read_file(nuts2_file).set_index("NUTS_ID").geometry
    )
    #nuts2 = gpd.GeoDataFrame(
...
def convert_nuts2_to_regions_yields(df_nuts2, regions, yield_cols=None):
    ...
    # If nothing overlaps, just return empty
    if overlay.empty:
        return pd.DataFrame(index=regs["name"].values, columns=yield_cols, dtype=float)

    # 4) Area of intersections (m² → km² if you like, but cancels in ratio)
    ...
    # 8) Ensure we have one row per region (regions with no valid overlaps → NaN)
...
if __name__ == "__main__":
    ...
    #regions = gpd.read_file('debug_pypsa_eur/perennials/regions_onshore_base_s_90.geojson')
    nuts2 = build_nuts2_shapes()

    yields = pd.read_csv(snakemake.input.perennials_yields_1G_biofuels, index_col = 0)
    #yields = pd.read_csv('debug_pypsa_eur/perennials/yields_perennials_1G_biofuels.csv', index_col = 0)

    df_nuts2 = gpd.GeoDataFrame(nuts2.geometry).join(yields)

    # Adress the missign countries (df_nuts2 contains all NUTS2 + missing shapes (with NaNs))
